...
async function webfinger(r) {
    try {
      const res = await ngx.fetch("https://social.memes.nz/.well-known/webfinger?" + r.variables.args)
      if(!res.ok){
        r.return(res.status)
        return
      }
      const js = await res.json()
      js.links.push({"rel": "http://openid.net/specs/connect/1.0/issuer", "href": "https://account.memes.nz/realms/master"})
